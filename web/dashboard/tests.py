...
class MovementHistoryTests(TestCase):
    ...
    def test_essence_reward_reports_quantity_instead_of_card_or_eur(self):
        ...
        MovementSnapshot.objects.create(user=self.user, movements=[movement], source_version=17)
    ...
    def test_crafting_chest_creates_an_owned_card_movement(self):
        card = self._api_card(player="Canales", rarity="limited")
        card.update({
            "assetId": "crafted-canales-165",
            "serialNumber": 165,
            "ownerSince": "2026-08-26T10:00:00Z",
            "ownershipHistory": [{
                "from": "2026-08-26T10:00:00Z", "transferType": "MINT",
                "user": {"slug": "burguis"},
            }],
            "tokenOwner": {"from": "2026-08-26T10:00:00Z", "user": {"slug": "burguis"}},
            "lowestPriceCard": {
                "assetId": "floor-canales",
                "liveSingleSaleOffer": {
                    "receiverSide": {"amounts": {"eurCents": 1234, "referenceCurrency": "EUR"}},
                },
            },
        })

        movements = _crafting_movements_from_chests([{
            "id": "chest-canales", "rarity": "limited", "cardShardsCount": 1000,
            "poolKeyShardsCounts": [
                {"slug": "laliga", "displayName": "LaLiga", "count": 500, "logoUrl": "laliga.png"},
                {"slug": "champions", "displayName": "Champions", "count": 500, "logoUrl": "champions.png"},
            ],
            "card": card,
        }], "burguis")

        self.assertEqual(len(movements), 1)
        self.assertEqual(movements[0]["category"], "crafting")
        self.assertEqual(movements[0]["essence_spent"], 1000)
        self.assertEqual(
            [(item["label"], item["count"]) for item in movements[0]["essence_breakdown"]],
            [("LaLiga", 500), ("Champions", 500)],
        )
        self.assertEqual(movements[0]["craft_floor_eur"], 12.34)
        self.assertTrue(movements[0]["craft_owned_by_me"])
        self.assertEqual(movements[0]["occurred_at"], "2026-08-26T10:00:00Z")
        self.assertEqual(movements[0]["cards"][0]["asset_id"], "crafted-canales-165")

    def test_crafting_history_links_the_exact_card_to_its_later_sale(self):
        card = {
            "asset_id": "crafted-canales-165", "player": "Canales", "rarity": "limited",
            "in_season": True, "serial_number": 165,
        }
        crafted = {
            "id": "craft:canales", "occurred_at": "2026-08-26T10:00:00Z",
            "direction": "craft", "category": "crafting", "cards": [card],
            "craft_owned_by_me": False, "essence_spent": 20,
        }
        sale = {
            "id": "sale:canales", "occurred_at": "2026-08-27T06:19:00Z",
            "direction": "sale", "cash_direction": "sale", "market": "Compra instantánea",
            "category": "laliga_inseason", "cards": [card], "sent_cards": [card],
            "received_cards": [], "net_eur": 6.27, "fee_eur": .33,
        }

        row = build_crafting_history([sale, crafted])[0]

        self.assertEqual(row["craft_status"], "sold")
        self.assertEqual(row["sale_market"], "Compra instantánea")
        self.assertEqual(row["sale_net_eur"], Decimal("6.27"))
        self.assertEqual(row["sale_fee_eur"], Decimal("0.33"))

    def test_crafting_history_keeps_owned_cards_without_a_disposal(self):
        card = {"asset_id": "crafted-owned", "player": "Carta abierta", "rarity": "rare"}
        crafted = {
            "id": "craft:owned", "occurred_at": "2026-08-25T10:00:00Z",
            "direction": "craft", "category": "crafting", "cards": [card],
            "craft_owned_by_me": True,
        }

        row = build_crafting_history([crafted])[0]

        self.assertEqual(row["craft_status"], "held")
    ...
    def test_pending_trade_cards_are_enriched_with_the_current_floor(self, request):
        bought = {
            "asset_id": "bought", "player": "Jugador vendido", "rarity": "rare",
            "in_season": True, "serial_number": 1,
        }
        received = {
            "asset_id": "received", "player": "Carta pendiente", "rarity": "limited",
            "in_season": False, "serial_number": 20,
        }
        movements = [
            {
                "id": "buy", "occurred_at": "2026-08-20T10:00:00Z",
                "direction": "purchase", "cash_direction": "purchase",
                "cards": [bought], "received_cards": [bought], "sent_cards": [],
                "gross_eur": 5,
            },
            {
                "id": "trade", "occurred_at": "2026-08-21T10:00:00Z",
                "direction": "trade", "cash_direction": "sale",
                "cards": [bought, received], "sent_cards": [bought], "received_cards": [received],
                "net_eur": 6,
            },
        ]
        request.return_value = {"tokens": {"anyCards": [{
            "assetId": "received",
            "lowestPriceCard": {"liveSingleSaleOffer": {"receiverSide": {"amounts": {
                "eurCents": 1234, "wei": "0", "referenceCurrency": "EUR",
            }}}},
        }]}}

        enriched = enrich_pending_card_floors(movements, headers={"Authorization": "test"})

        self.assertEqual(enriched, 1)
        self.assertEqual(received["market_floor_eur"], 12.34)
        self.assertEqual(request.call_args.args[1], {"assetIds": ["received"]})
    ...
    def test_collector_uses_completed_trades_and_discards_open_auctions(self, request):
        ...
        def response(query, variables, headers=None):
            ...
            if "CraftedCards" in query:
                return {"currentUser": {"slug": "burguis", "cardShardsChests": []}}
    ...
    def test_background_sync_replaces_snapshot_only_after_success(self, collect):
        ...
        self.assertEqual(snapshot.source_version, 17)
    ...
    def test_reward_filter_separates_money_essence_and_cards(self):
        ...
        MovementSnapshot.objects.create(user=self.user, movements=[money, essence, card], source_version=17)
    ...
    def test_reward_kpis_sum_money_essence_by_rarity_and_cards(self):
        ...
        MovementSnapshot.objects.create(
            user=self.user, source_version=17,
    ...
    def test_trading_shortcut_only_shows_completed_purchase_sale_cycles(self):
        canales = {
            "asset_id": "canales-165", "player": "Canales", "team": "Racing",
            "rarity": "limited", "in_season": True, "serial_number": 165,
        }
        pending = {
            "asset_id": "pending", "player": "Aún en galería", "team": "Racing",
            "rarity": "rare", "in_season": True, "serial_number": 20,
        }
        lejeune = {
            "asset_id": "lejeune", "player": "Florian Lejeune", "team": "Rayo Vallecano",
            "rarity": "limited", "in_season": False, "serial_number": 42,
            "market_floor_eur": 3.45,
        }
        MovementSnapshot.objects.create(user=self.user, source_version=17, movements=[
            {
                "id": "canales-buy", "occurred_at": "2026-08-26T12:33:00Z",
                "direction": "purchase", "cash_direction": "purchase", "market": "Oferta pública",
                "category": "laliga_inseason", "cards": [canales], "received_cards": [canales],
                "sent_cards": [], "gross_eur": 4.98, "net_eur": 4.98,
            },
            {
                "id": "canales-sale", "occurred_at": "2026-08-27T06:19:00Z",
                "direction": "trade", "cash_direction": "sale", "market": "Intercambio + dinero",
                "category": "laliga_inseason", "cards": [canales, lejeune], "received_cards": [lejeune],
                "sent_cards": [canales], "gross_eur": 6.60, "net_eur": 6.27, "fee_eur": .33,
            },
            {
                "id": "pending-buy", "occurred_at": "2026-08-27T08:00:00Z",
                "direction": "purchase", "cash_direction": "purchase", "market": "Subasta",
                "category": "laliga_inseason", "cards": [pending], "received_cards": [pending],
                "sent_cards": [], "gross_eur": 5, "net_eur": 5,
            },
        ])

        response = self.client.get(reverse("movements"), {"category": "trading", "date_from": ""})

        self.assertEqual(response.context["total_rows"], 1)
        self.assertEqual(response.context["trading_count"], 1)
        self.assertContains(response, "Compraventas")
        self.assertContains(response, "Canales")
        self.assertContains(response, "+1,29 €")
        self.assertEqual(response.context["totals"]["trading_balance"], Decimal("1.29"))
        self.assertEqual(response.context["totals"]["trading_pending_cards"], 1)
        self.assertEqual(response.context["totals"]["trading_pending_value"], Decimal("3.45"))
        self.assertContains(response, "1 carta")
        self.assertContains(response, "Florian Lejeune")
        self.assertContains(response, "3,45 €")
        self.assertContains(response, "Valor observable")
        self.assertNotContains(response, "operaciones contabilizadas")
        self.assertNotContains(response, ">Compras<")
        self.assertNotContains(response, "Aún en galería")

    def test_crafting_shortcut_shows_held_and_sold_cards_with_the_exit_method(self):
        held = {
            "asset_id": "crafted-held", "player": "Carta conservada", "team": "Racing",
            "rarity": "rare", "in_season": True, "serial_number": 8,
        }
        sold = {
            "asset_id": "crafted-sold", "player": "Carta vendida", "team": "Racing",
            "rarity": "limited", "in_season": True, "serial_number": 165,
        }
        MovementSnapshot.objects.create(user=self.user, source_version=17, movements=[
            {
                "id": "craft:held", "occurred_at": "2026-08-25T10:00:00Z",
                "direction": "craft", "cash_direction": "other", "market": "Crafting con esencia",
                "category": "crafting", "cards": [held], "received_cards": [held], "sent_cards": [],
                "craft_owned_by_me": True, "essence_spent": 1000, "craft_floor_eur": 12.34,
                "essence_breakdown": [
                    {"slug": "laliga", "label": "LaLiga", "count": 500, "logo_url": ""},
                    {"slug": "champions", "label": "Champions", "count": 500, "logo_url": ""},
                ],
            },
            {
                "id": "craft:sold", "occurred_at": "2026-08-24T10:00:00Z",
                "direction": "craft", "cash_direction": "other", "market": "Crafting con esencia",
                "category": "crafting", "cards": [sold], "received_cards": [sold], "sent_cards": [],
                "craft_owned_by_me": False, "essence_spent": 1000, "craft_floor_eur": 9.87,
                "essence_breakdown": [],
            },
            {
                "id": "crafted-sale", "occurred_at": "2026-08-27T06:19:00Z",
                "direction": "sale", "cash_direction": "sale", "market": "Compra instantánea",
                "category": "laliga_inseason", "cards": [sold], "received_cards": [],
                "sent_cards": [sold], "gross_eur": 6.60, "net_eur": 6.27, "fee_eur": .33,
            },
        ])

        response = self.client.get(reverse("movements"), {"category": "crafting", "date_from": ""})

        self.assertEqual(response.context["total_rows"], 2)
        self.assertEqual(response.context["crafting_count"], 2)
        self.assertEqual(response.context["totals"]["craft_held"], 1)
        self.assertEqual(response.context["totals"]["craft_sold"], 1)
        self.assertContains(response, "Carta conservada")
        self.assertContains(response, "La sigues teniendo")
        self.assertContains(response, "Carta vendida")
        self.assertContains(response, "Compra instantánea")
        self.assertContains(response, "+6,27 €")
        self.assertContains(response, "500")
        self.assertContains(response, "LaLiga")
        self.assertContains(response, "Champions")
        self.assertContains(response, "12,34 €")
        self.assertContains(response, "9,87 €")
        self.assertNotContains(response, "1000 esencia")
...
class OpportunityMarketTests(TestCase):
    ...
    def test_page_filters_cached_opportunities_without_calling_sorare(self):
        ...
        self.assertEqual(response.context["focus_rarity"], "rare")
        self.assertEqual(response.context["sort"], "discount")
        self.assertEqual(response.context["page_obj"][0]["player"], "Oportunidad Roja")
        self.assertContains(response, "Operar")
        self.assertContains(response, "Compra instantánea")
        self.assertContains(response, "Ofertas recibidas")
        self.assertContains(response, "Configuración")
        self.assertContains(response, "Token de Sorare")
